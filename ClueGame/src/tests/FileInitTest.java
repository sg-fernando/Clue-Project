...
public class FileInitTest {
	...
	public void testRoomLabels() {
		...
		assertEquals("Sophia's Bedroom", board.getRoom('S').getName() );
		assertEquals("Rose's Bedroom", board.getRoom('R').getName() );
		assertEquals("Bathroom", board.getRoom('W').getName() );
		assertEquals("Dorothy's Bedroom", board.getRoom('D').getName() );
	...
	public void testRooms() {
		...
		BoardCell cell = board.getCell(19, 18);
		Room room = board.getRoom( cell ) ;
		assertTrue( room != null );
		assertEquals( room.getName(), "Blanche's Room" ) ;
		assertFalse( cell.isLabel() );
		assertFalse( cell.isRoomCenter() ) ;
		assertFalse( cell.isDoorway()) ;

		// this is a label cell to test
		cell = board.getCell(13, 24);
		room = board.getRoom( cell ) ;
		assertTrue( room != null );
		assertEquals( room.getName(), "Front Yard" ) ;
